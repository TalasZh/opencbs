...
close_date = DATEADD(month,ISNULL((SELECT nb_of_installment FROM Credit WHERE Credit.id = Contracts.id),0),start_date)
GO

UPDATE Contracts SET
close_date = (SELECT MAX(paid_date) FROM Installments WHERE Installments.contract_id = Contracts.id)
WHERE (SELECT SUM(Installments.capital_repayment - Installments.paid_capital) FROM Installments 
WHERE Installments.contract_id = Contracts.id) < 0.03
GO
...
UPDATE [TechnicalParameters] SET [value]='v1.1.1'
GO
